...
async function run() {
  ...
  global.fetch = async (url, options) => {
    ...
    return Response.json({ key: "test+key=", token: `test/token-${calls.length}` });
  };
  ...
  const firstAttempt = await db("payment_attempts").where({ invoice_id: invoiceId }).first();
  await db("payment_attempts").where({ id: firstAttempt.id }).update({ created_at: new Date(Date.now() - 5000) });
  response = { status: "failed", code: 600, sid: null, response: "order_id not found" };
  const reopened = await startCheckoutAction(invoiceId);
  assert.ok(reopened.url);
  assert.notEqual(reopened.url, attempts.find((item) => item.url).url, "Back then Pay must request a fresh token.");
  assert.equal(calls.length, 3, "Retry must verify the old order, then create a new checkout.");
  assert.equal((await db("payment_attempts").where({ id: firstAttempt.id }).first()).status, "failed");
  const attempt = await db("payment_attempts").where({ invoice_id: invoiceId, status: "ready" }).first();
  assert.notEqual(attempt.id, firstAttempt.id, "Each checkout must retain a distinct verifiable order.");
  await db("payment_attempts").where({ id: attempt.id }).update({ created_at: new Date(Date.now() - 5000) });
  response = { status: "pending", code: 602 };
  const pendingRetry = await startCheckoutAction(invoiceId);
  assert.ok(pendingRetry.message);
  assert.equal(pendingRetry.url, undefined, "Pending payments must not issue another token.");
  await db("payment_attempts").where({ id: attempt.id }).update({ checked_at: null });
  response = { sid: `sid-${suffix}`, status: "success", code: 601, amount: "0.99" };
  ...
  assert.equal((await verifyPaymentAction(invoiceId)).retryable, true);
  gatewayTimeout = true;
