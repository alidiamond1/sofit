...
Checkout creation is serialized per client. Repeated clicks reuse the checkout. An uncertain network result stays pending until verification; it never automatically creates a second charge. If Sifalo cannot resolve an unknown order or an expired checkout, reconcile it in the merchant portal before allowing another attempt. Do not manually mark an invoice paid from a return URL or screenshot.
