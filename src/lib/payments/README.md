...
The return URL is `/client/payments?order_id=<our UUID>`. Sifalo appends `sid`; the application deliberately ignores browser-supplied `sid`, status and amount. It verifies using the stored order ID through Sifalo's authenticated Verify API, checks success/code and the gross or exact fee-adjusted amount, and enforces a unique transaction ID in MySQL. Each attempt has a different order ID. No undocumented webhook or signature format is assumed.
...
Successful Sifalo verification may report the net settlement rather than the gross checkout amount. The merchant-confirmed 0.40% rate is applied with integer-cent arithmetic; only the exact gross or exact expected net is accepted, alongside the stored unique order, USD, success/code 601 and unique SID. `SIFALO_TRANSACTION_FEE_PERCENT` overrides the account rate when needed. No fee controls are added to the client UI.

The Payments page automatically reconciles its stored checkout after a return or manual navigation, retries a pending provider response up to three times, and refreshes the authenticated layout. A confirmation dialog appears once per invoice per browser session only when the server reports paid access. Query parameters alone never open menus or plans.
